...
mod tests {
    ...
    fn test_slack_token_detection() {
        // Built at compile time so push protection does not flag literal token shapes in source.
        let slack_bot = concat!(
            "xox", "b-", "000000000000-0000000000000-0000000000000000"
        );
        let slack_user = concat!(
            "xox", "p-", "000000000000-0000000000000-0000000000000000"
        );
        let slack_app = concat!("xapp-1-A00000000-0000000000000-00000000");
        assert!(contains_pii(slack_bot));
        assert!(contains_pii(slack_user));
        assert!(contains_pii(slack_app));

        let result = remove_pii(concat!(
            "SLACK_TOKEN=xox",
            "b-000000000000-0000000000000-000"
        ));
    ...
    fn test_discord_token_detection() {
        ...
        assert!(contains_pii(discord_shape));
    }
    ...
    fn test_performance_with_all_new_patterns() {
        ...
        let input = concat!(
            "Email: test@example.com, postgres://user:pass@host/db, ",
            "key: sk-ant-api03-abc123, IBAN: DE89370400440532013000, ",
            "-----BEGIN RSA PRIVATE KEY-----, xox",
            "b-token-here-1234567890"
        );
