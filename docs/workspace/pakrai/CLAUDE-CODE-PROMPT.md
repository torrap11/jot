...
- Do **not** ask the human to approve routine `cargo`, `npm`, `bun`, `curl`, or file edits under `jot/`, `integration/`, `engine/`.
...
**In scope:** screen recording + query via `integration/` + `engine/` engine (`:3030`).
...
```bash
cd screenpipe-x-jot
test -f jot/package.json && test -f engine/Cargo.toml && test -d integration
./scripts/build-engine.sh    # if engine/target/release/screenpipe missing
cd . && npm test
...
Screenpipe API reference: `engine/.claude/skills/screenpipe-api/SKILL.md`
